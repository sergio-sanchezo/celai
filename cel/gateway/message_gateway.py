...
class MessageGateway:
    ...
    def __init__(
        ...
        message_enhancer: Callable[
            [ConversationLead, str, bool], OutgoingMessage
        ] = None,
            ...
            async def secure_middleware(request: Request, call_next):
                ...
                if path_is_secure(request.url.path):
                    if gateway_api_key_header not in request.headers:
                        ...
                        log.warning(
                            f"Add your API Key into header: {gateway_api_key_header}"
                        )
                        return response
    ...
    def register_middleware(
        self, middleware: Callable[[Message, BaseConnector, BaseAssistant], bool]
    ):
    ...
    async def __process_events(self, message: Message):
        try:
            ...
            if res is None:
                log.debug(
                    f"Event: 'message' response is None for message: {message.text}"
                )
                return True
    ...
    async def __process_client_command(self, message: Message):
        ...
        cmd_executed = not await self.assistant.eval_client_command(
            command=command, args=args, message=message
        )
    ...
    async def process_message(
        ...
        if not status:
            log.warning(
                f"Message {message.lead.get_session_id()} rejected by middlewares"
            )
            if custom_message:
                rejection_chunk = StreamContentChunk(
                    content=custom_message, is_partial=False
                )
        ...
        if self.assistant:
            ...
            if mode == StreamMode.SENTENCE:
                async for sentence in streaming_sentence_detector_async(stream):
                    assert isinstance(
                        sentence, StreamContentChunk
                    ), "stream must be a StreamChunk"
                    ...
                    if capture_repsonse:
                        ...
                        await self.dispatch_outgoing_genai_message(
                            message.lead,
            ...
            if mode == StreamMode.FULL:
                ...
                async for chunk in stream:
                    ...
                    if not capture_repsonse:
                        ...
                        await self.dispatch_outgoing_genai_message(
                            message.lead, text=content, is_partial=False
                        )
            ...
            if capture_repsonse:
                ...
                await connector.send_text_message(
                    message.lead, text="No assistant available"
                )
    ...
    async def send_text_message(
        ...
        assert source_message is None or isinstance(
            source_message, Message
        ), "source_message must be an instance of Message"
    ...
    async def dispatch_outgoing_genai_message(
        self, lead: ConversationLead, text: str, is_partial=False, is_truncated=False
    ):
        ...
        assert isinstance(
            lead.connector, BaseConnector
        ), "connector must be an instance of BaseConnector"
        connector = lead.connector

        # TODO: Call outgoing message enhancer
        msg = await self.message_enhancer(lead, text, is_partial, is_truncated)
    ...
    def register_connector(self, connector: BaseConnector):
        ...
        assert (
            conn is None or isinstance(conn, APIRouter) or isinstance(conn, FastAPI)
        ), "Router must be an instance of APIRouter or FastAPI"
    ...
    def run(self, enable_ngrok: bool = False):
        ...
        try:
            ...
            log.error(
                "Uvicorn is not installed. Please install uvicorn to run the gateway"
            )
            return
