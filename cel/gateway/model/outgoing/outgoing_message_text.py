...
from cel.gateway.model.outgoing.outgoing_message import (
    OutgoingMessage,
    OutgoingMessageType,
)
...
class OutgoingTextMessage(OutgoingMessage):
    ...
    def from_dict(data: dict) -> "OutgoingTextMessage":
        ...
        assert "content" in data, "data must have a 'content' key"
        assert "lead" in data, "data must have a 'lead' key"
        assert isinstance(
            data["lead"], ConversationLead
        ...
        return OutgoingTextMessage(
            ...
            attachments=[
                FileAttachment.from_dict(attachment)
